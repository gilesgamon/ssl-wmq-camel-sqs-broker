...
resource "aws_lambda_function" "rest_handler_function" {
  filename = data.archive_file.rest_handler.output_path
  function_name = "rest_handler_function"
...
resource "aws_lambda_function" "mq_function" {
  filename = data.archive_file.sqsToDdb.output_path
  function_name = "mq_function"
...
resource "aws_lambda_event_source_mapping" "mq_function" {
  event_source_arn = aws_sqs_queue.mqToSqsQueue.arn
  function_name    = aws_lambda_function.mq_function.arn
}
...
resource "aws_lambda_permission" "apigw_lambda" {
  ...
  function_name = aws_lambda_function.rest_handler_function.function_name
