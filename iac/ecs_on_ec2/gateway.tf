...
resource "aws_api_gateway_integration" "rest_handler_method-integration" {
  ...
  type = "AWS_PROXY"
  uri = aws_lambda_function.rest_handler_function.invoke_arn
...
output "dev_url" {
  value = "https://${aws_api_gateway_deployment.deployment_dev.rest_api_id}.execute-api.${var.region}.amazonaws.com/${aws_api_gateway_stage.test.stage_name}"
}
